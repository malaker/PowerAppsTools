...
{
    public class GetBackupsCmdletStub : GetBackupsCmdlet
    {
        public GetBackupsCmdletStub(IXrmOnlineManagementApiClient client)
        {
            this._client = client;
        }
    ...
    public class GetBackupsCmdletTests
    {
        [Fact]
        public void ShouldCmdletBeInvoked()
        {
            ...
            client.Setup(m => m.GetInstanceBackups(It.IsAny<GetInstanceBackups>(), It.IsAny<CancellationToken>())).Returns(Task.FromResult(expectedResponse));

            var sut = new GetBackupsCmdletStub(client.Object)
            {
            ...
            var results = sut.Invoke().OfType<GetInstanceBackupsResponse>().ToList();
